...
window.CONTENT_ITEMS = [
  ...
  },
  {
    ...
    description: "GPU利用率不足是许多生产环境深度学习集群面临的重要问题，会导致作业排队时间延长和运营成本增加。GPU共享是缓解该低效问题的一种有前景的方案，它允许多个工作负载在单块GPU上并发执行，从而提高资源利用率。然而，现有GPU共享机制存在集成成本高、性能隔离不足以及…",
  ...
  },
  {
    ...
    description: "大语言模型（LLM）服务需要同时满足预填充（prefill）和解码（decode）阶段严格的服务级别目标（SLO）。现有方案中，一些将两个阶段分离部署，可能导致资源闲置或计算冗余；另一些将预填充阶段切分为多个块并与解码迭代融合，造成SLO合规性与高利用率之间的…",
